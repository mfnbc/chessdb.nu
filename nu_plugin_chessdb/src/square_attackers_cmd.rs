...
use crate::core::square_attackers;
use crate::utils::json_to_nu_value;
use crate::ChessdbPlugin;
use crate::PLUGIN_CATEGORY;

/// Nu-facing exposure of `core::square_attackers` — every piece that attacks
/// a given square (occupancy-aware, turn-independent, works whether or not
/// the target square is occupied). The reverse question from
/// `chessdb square-control`'s "what does this piece see."
pub struct SquareAttackersCmd;

impl PluginCommand for SquareAttackersCmd {
    type Plugin = ChessdbPlugin;

    fn name(&self) -> &str {
        "chessdb square-attackers"
    }

    fn description(&self) -> &str {
        "Every white/black piece that attacks --square (occupancy-aware, independent of whose turn it is), for a FEN (pipeline input)."
    }
    ...
    fn run(
        ...
        let result = square_attackers(&fen, &square, span)?;
